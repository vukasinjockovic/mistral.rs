...
use mistralrs_quant::leech::{leech_decode_bf16, leech_gemv_bf16};
...
fn bench_gemv_bf16_batch1() {
    ...
    let tflops = flops_per_iter * iters as f64 / elapsed.as_secs_f64() / 1e12;
    ...
    let hbm_gbps = hbm_bytes_per_iter * iters as f64 / elapsed.as_secs_f64() / 1e9;
    ...
    eprintln!("  TFLOPS:        {:.3}", tflops);
    eprintln!("  HBM read:      {:.3} GB/s (packed_stream + activations)", hbm_gbps);
    eprintln!("  vs RTX 5090 HBM peak ~1.7 TB/s: {:.2}%", 100.0 * hbm_gbps * 1e9 / 1.7e12);
}
