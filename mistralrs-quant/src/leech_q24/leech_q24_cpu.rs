...
pub fn compute_substream_bit_offsets(
    tile_nb_totals: &[u16],
    _substream_nb_totals: &[u16],
    k: usize,
) -> Vec<u64> {
    // CPU stub: just sum aggregate nb_totals (sufficient at K=1 only).
...
pub fn validate_t_k_pair(_tile_size: i32, _num_streams: i32) -> Result<(), LeechQ24DecodeError> {
    Err(LeechQ24DecodeError::CpuNotImplemented)
}
...
pub unsafe fn leech_q24_gemv_bf16_v4(
    _a_act_bf16_ptr: *const c_void,
    _packed_buckets: *const u8,
    _substream_states: *const u16,
    _substream_nb_totals: *const u16,
    _tile_bitstream: *const u64,
    _substream_bit_offsets: *const u64,
    _beta_idx_packed: *const u8,
    _offset_idx_packed: *const u8,
    _beta_lloyd_ptr: *const f32,
    _offset_lloyd_ptr: *const f32,
